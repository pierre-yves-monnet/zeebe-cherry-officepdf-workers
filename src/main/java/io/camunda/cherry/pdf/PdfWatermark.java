...
@Component
public class PdfWatermark extends PdfWorker {

    /**
     * Different BPMN Error
     */
    ...
    private static final String INPUT_WATERMARK = "watermark";
    private static final String INPUT_WATERMARK_POSITION = "watermarkPosition";
    private static final String INPUT_WATERMARK_COLOR = "watermarkColor";
    private static final String INPUT_WATERMARK_ROTATION = "watermarkRotation";
    ...
    public PdfWatermark() {
        super(WORKERTYPE_PDF_WATERMARK,
                Arrays.asList(
                        RunnerParameter.getInstance(INPUT_SOURCE_FILE, "Source file", Object.class,
                                RunnerParameter.Level.REQUIRED, "FileVariable for the file to convert"),
                        RunnerParameter.getInstance(INPUT_WATERMARK, "Watermark", String.class,
                                RunnerParameter.Level.REQUIRED, "Watermark to add"),
                        RunnerParameter.getInstance(INPUT_WATERMARK_POSITION, "Watermark position",
                                        String.class,
                                        PdfToolbox.TEXT_POSITION.CENTER.toString(),
                                        RunnerParameter.Level.OPTIONAL,
                                        "Text position (" + PdfToolbox.TEXT_POSITION.TOP + ", " + PdfToolbox.TEXT_POSITION.CENTER + "," + PdfToolbox.TEXT_POSITION.BOTTOM + ")")
                                .addChoice("TOP", PdfToolbox.TEXT_POSITION.TOP.toString())
                                .addChoice("CENTER", PdfToolbox.TEXT_POSITION.CENTER.toString())
                                .addChoice("BOTTOM", PdfToolbox.TEXT_POSITION.BOTTOM.toString()),
                        RunnerParameter.getInstance(INPUT_WATERMARK_COLOR, "Color", String.class,
                ...
                Collections.singletonList(
                        RunnerParameter.getInstance(OUTPUT_DESTINATION_FILE, "Output destination file", Object.class, RunnerParameter.Level.REQUIRED, "FileVariable converted")
                ),
                Arrays.asList(BpmnError.getInstance(BPMERROR_ENCRYPTED_NOT_SUPPORTED, "PDF Encripted file not supported"),
                        ...
                        BpmnError.getInstance(BPMERROR_INVALID_COLOR, "Invalid color"),
                        BpmnError.getInstance(BPMERROR_OPERATION_ERROR, "Operation error")));
    }
    ...
    public String getDescription() {
        return "Place a watermark in a PDF file. Different positions are possible: TOP, CENTER, BOTTOM. On the CENTER position, a rotation can be provided.\n" +
                "Color of the watermark is possible too.";
    }
     ...
     * @param activatedJob     Activated job to get information (variables..)
    ...
    public void execute(final JobClient jobClient, final ActivatedJob activatedJob, ContextExecution contextExecution) {
        ...
        try {
            String watermark = getInputStringValue(INPUT_WATERMARK, "Watermark", activatedJob);
            PdfToolbox.WriterOption writerOption = PdfToolbox.WriterOption.getInstance();

            PdfToolbox.TEXT_POSITION textPosition = PdfToolbox.TEXT_POSITION.valueOf(getInputStringValue(INPUT_WATERMARK_POSITION, PdfToolbox.TEXT_POSITION.CENTER.toString(), activatedJob));
            // textPosition is never null
            writerOption.setTextPosition(textPosition);

            String watermarkColorSt = getInputStringValue(INPUT_WATERMARK_COLOR, null, activatedJob);
            if (watermarkColorSt != null) {
                Color color = getColorFromString(watermarkColorSt);
                if (color == null)
                    throw new ZeebeBpmnError(BPMERROR_INVALID_COLOR, "Worker [" + getName() + "] Color [" + watermarkColorSt + "] is unknow]");

                writerOption.setColor(color);
            }
            Long watermarkRotation = getInputLongValue(INPUT_WATERMARK_ROTATION, null, activatedJob);
            ...
            for (int i = 0; i < sourceDocument.getNumberOfPages(); i++) {
                PdfToolbox.addWatermarkText(sourceDocument, sourceDocument.getPage(i),
                        writerOption,
                        watermark);
            }

            saveOutputPdfDocument(sourceDocument, destinationFileName,
            ...
            throw new ZeebeBpmnError(BPMERROR_OPERATION_ERROR, "Worker [" + getName() + "] cannot execute watermark operation on [" + sourceFileVariable.name + "] : " + e);
        ...
        logInfo("Cherry.PdfWatermark: from document[" + sourceFileVariable.name + "] to [" + destinationFileName + "]");
    }
